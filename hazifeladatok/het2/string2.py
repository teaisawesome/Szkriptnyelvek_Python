...
def not_bad(s):
    ...
    if nindex < bindex:
        s = s.replace(s[nindex:bindex+3], 'good')
...
def front_back(a, b):
    ...
    if len(a) % 2 == 0:
        resultA = a[0:int(len(a)/2)]
        resultB = a[int(len(a)/2):]
    else:
        resultA = a[0:int(len(a)/2)+1]
        resultB = a[int(len(a)/2)+1:]

    if len(b) % 2 == 0:
        resultA = resultA + b[0:int(len(b)/2)]
        resultB = resultB + b[int(len(b)/2):]
    else:
        resultA = resultA + b[0:int(len(b)/2)+1]
        resultB = resultB + b[int(len(b)/2)+1:]
